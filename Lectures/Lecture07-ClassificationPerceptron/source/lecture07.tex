...
\begin{frame}
    ...
    Begin with $W^{(1)} = (0,0,0)$. On next step: $W^{(2)} = \vec{0} + y_1X_1 = -1\ast(-1,3,1) = (1,-3,-1)$.

    Next: since $y_1W^{(2)}\cdot X_1 > 0$, check $y_2W^{(2)}\cdot X_2 = -1\ast(-1 + 3 -1) = -1$. So, 
        \[W^{(3)} = W^{(2)} + y_2X_2 = (2, -2, -2).\]
    Continue in this way {--} on each step check dot products (in order) with $X_1, X_2, X_3, X_4$. Eventually you get, 
        \[W^{(10)} = (4, -0.5, 1)\]
    and this vector is returned, meaning that $H = \{(x_1,x_2)\in \mathbb R^2:\ 4x_1-0.5x_2 + 1 = 0\}$ separates the points.
\end{frame}
